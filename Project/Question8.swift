...
class Question8: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        self.view.backgroundColor = UIColor(red: CGFloat(0), green: CGFloat(204)/255, blue: CGFloat(255)/255, alpha: CGFloat(1.0))
        countDownLabel.center.x = self.view.bounds.width/2
        Question8.lineBreakMode = NSLineBreakMode.ByWordWrapping
        Question8.sizeToFit()
        name.text = player1Name
        name2.text = player2Name
        name.sizeToFit()
        name2.sizeToFit()
        Question8.center.x = self.view.bounds.width/2
    ...
    override func viewDidAppear(animated: Bool) {
        ...
        UIView.animateWithDuration(0.3, delay: 2.5, options: [], animations: {self.Question8.alpha += 1}, completion: nil)
    ...
    override func prepareForSegue(segue: UIStoryboardSegue, sender: AnyObject?) {
        if(segue.identifier == "segueChangeQ9"){
            let svc = segue.destinationViewController as! Question9;
