...
class Question35: UIViewController {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        self.view.backgroundColor = UIColor(red: CGFloat(0), green: CGFloat(153)/255, blue: CGFloat(153)/255, alpha: 1.0)
        Question35.sizeToFit()
        name.text = player1Name
        name2.text = player2Name
        name.sizeToFit()
        name2.sizeToFit()
        Question35.lineBreakMode = NSLineBreakMode.ByWordWrapping
        Question35.center.x = self.view.bounds.width/2
    ...
    override func viewDidAppear(animated: Bool) {
        ...
        UIView.animateWithDuration(0.3, delay: 2.5, options: [], animations: {self.Question35.alpha += 1}, completion: nil)
    ...
    override func prepareForSegue(segue: UIStoryboardSegue, sender: AnyObject?) {
        if(segue.identifier == "segueChangeQ36"){
            let svc = segue.destinationViewController as! Question36;
