...
class Question15: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        self.view.backgroundColor = UIColor(red: CGFloat(153)/255, green: CGFloat(204)/255, blue: CGFloat(0)/255, alpha: 1.0)
        Question15.center.x = self.view.bounds.width/2
        Question15.sizeToFit()
        name.text = player1Name
        name2.text = player2Name
        name.sizeToFit()
        name2.sizeToFit()
        Question15.lineBreakMode = NSLineBreakMode.ByWordWrapping
    ...
    override func viewDidAppear(animated: Bool) {
        ...
        UIView.animateWithDuration(0.3, delay: 2.5, options: [], animations: {self.Question15.alpha += 1}, completion: nil)
    ...
    override func prepareForSegue(segue: UIStoryboardSegue, sender: AnyObject?) {
        if(segue.identifier == "segueChangeQ16"){
            let svc = segue.destinationViewController as! Question16;
