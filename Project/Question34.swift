...
class Question34: UIViewController {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        self.view.backgroundColor = UIColor(red: CGFloat(51)/255, green: CGFloat(119)/255, blue: CGFloat(255)/255, alpha: 1.0)
        Question34.sizeToFit()
        name.text = player1Name
        name2.text = player2Name
        name.sizeToFit()
        name2.sizeToFit()
        Question34.lineBreakMode = NSLineBreakMode.ByWordWrapping
        Question34.center.x = self.view.bounds.width/2
    ...
    override func viewDidAppear(animated: Bool) {
        ...
        UIView.animateWithDuration(0.3, delay: 2.5, options: [], animations: {self.Question34.alpha += 1}, completion: nil)
    ...
    override func prepareForSegue(segue: UIStoryboardSegue, sender: AnyObject?) {
        if(segue.identifier == "segueChangeQ35"){
            let svc = segue.destinationViewController as! Question35;
