...
class Question33: UIViewController {
    ...
    @IBOutlet weak var Question33: UILabel!
    
    @IBOutlet weak var nextQuestion: UIButton!
    override func viewDidLoad() {
        super.viewDidLoad()
        self.view.backgroundColor = UIColor(red: CGFloat(0), green: CGFloat(204)/255, blue: CGFloat(0), alpha: 1.0)
        Question33.center.x = self.view.bounds.width/2
        Question33.sizeToFit()
        name.text = player1Name
        name2.text = player2Name
        name.sizeToFit()
        name2.sizeToFit()
        Question33.lineBreakMode = NSLineBreakMode.ByWordWrapping
    ...
    override func viewDidAppear(animated: Bool) {
        ...
        UIView.animateWithDuration(0.3, delay: 2.5, options: [], animations: {self.Question33.alpha += 1}, completion: nil)
    ...
    override func prepareForSegue(segue: UIStoryboardSegue, sender: AnyObject?) {
        if(segue.identifier == "segueChangeQ34"){
            let svc = segue.destinationViewController as! Question34;
