...
class Question31: UIViewController {
    ...
    @IBOutlet weak var Question31: UILabel!
    
    @IBOutlet weak var nextQuestion: UIButton!
    override func viewDidLoad() {
        super.viewDidLoad()
        self.view.backgroundColor = UIColor(red: CGFloat(102)/255, green: CGFloat(102)/255, blue:CGFloat(255)/255, alpha: 1.0)
        
        Question31.sizeToFit()
        name.text = player1Name
        name2.text = player2Name
        name.sizeToFit()
        name2.sizeToFit()
        Question31.lineBreakMode = NSLineBreakMode.ByWordWrapping
        Question31.center.x = self.view.bounds.width/2
    ...
    override func viewDidAppear(animated: Bool) {
        ...
        UIView.animateWithDuration(0.3, delay: 2.5, options: [], animations: {self.Question31.alpha += 1}, completion: nil)
    ...
    override func prepareForSegue(segue: UIStoryboardSegue, sender: AnyObject?) {
        if(segue.identifier == "segueChangeQ32"){
            let svc = segue.destinationViewController as! Question32;
