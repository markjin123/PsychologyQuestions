...
class Questions29: UIViewController {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        self.view.backgroundColor = UIColor(red: CGFloat(0), green: CGFloat(230)/255, blue: CGFloat(153)/255, alpha: 1.0)
        
        Question29.sizeToFit()
        name.text = player1Name
        name2.text = player2Name
        name.sizeToFit()
        name2.sizeToFit()
        Question29.lineBreakMode = NSLineBreakMode.ByWordWrapping
        Question29.center.x = self.view.bounds.width/2
    ...
    override func viewDidAppear(animated: Bool) {
        ...
        UIView.animateWithDuration(0.3, delay: 2.5, options: [], animations: {self.Question29.alpha += 1}, completion: nil)
    ...
    override func prepareForSegue(segue: UIStoryboardSegue, sender: AnyObject?) {
        if(segue.identifier == "segueChangeQ30"){
            let svc = segue.destinationViewController as! Question30;
