...
class Question30: UIViewController {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        self.view.backgroundColor = UIColor(red: CGFloat(255)/255, green: CGFloat(77)/255, blue: CGFloat(136)/255, alpha: 1.0)
        Question30.center.x = self.view.bounds.width/2
        Question30.sizeToFit()
        name.text = player1Name
        name2.text = player2Name
        name.sizeToFit()
        name2.sizeToFit()
        Question30.lineBreakMode = NSLineBreakMode.ByWordWrapping
    ...
    override func viewDidAppear(animated: Bool) {
        ...
        UIView.animateWithDuration(0.3, delay: 2.5, options: [], animations: {self.Question30.alpha += 1}, completion: nil)
    ...
    override func prepareForSegue(segue: UIStoryboardSegue, sender: AnyObject?) {
        if(segue.identifier == "segueChangeQ31"){
            let svc = segue.destinationViewController as! Question31;
